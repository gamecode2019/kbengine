...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientVersion = "2.5.2";
	...
	*/
	...
	this.Client_onLoginSuccessfully = function(args)
	{
		...
		//KBEngine.app.serverdatas = args.readBlob();
		//改
		var size = args.readUint16();
		console.info("????udp port####",size);
		KBEngine.app.serverdatas = KBEngine.utf8ArrayToString(args.readBlob());
	...
	this.onUpdatePropertys_ = function(eid, stream)
	{
		...
		// console.info("############::",stream,currModule,pdatas);
		while(stream.length() > 0)
		{
			var utype = 0;
			var child_utype = 0;
			if(currModule.usePropertyDescrAlias){
				...
				child_utype = stream.readUint8();
			}
			else{
				utype = stream.readUint16();
				child_utype = stream.readUint16()
			}
			...
			propertydata = pdatas[child_utype];
			var setmethod = propertydata[5];
			var flags = propertydata[6];
	...
	this.onRemoteMethodCall_ = function(eid, stream)
	{
		...
		var methoddata = KBEngine.moduledefs[entity.className].methods[child_utype];
		console.info("############::",stream,KBEngine.moduledefs,methodUtype,child_utype);
